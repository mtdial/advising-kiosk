...
function QueueTable({ rows, now }) {
  const cols = ['Student Name', 'Student Email', 'College', 'Advisor', 'Appt Type', 'Wait Time', 'Notes']
  return (
...
export default function EASuiteAdminPage() {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    const [activeRes, seenRes] = await Promise.all([
      supabase
        .from('queue')
        .select('*, college:colleges(name), advisor:advisors!inner(name, ea_suite)')
        .eq('advisor.ea_suite', true)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-xl font-bold text-[var(--primary)]">EA Suite Queue</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow p-16 text-center text-gray-400">
            No students currently waiting on advisors in the EA Suite.
          </div>
        ) : (
          <>
            {/* Live Queue */}
